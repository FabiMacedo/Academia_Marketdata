...
SELECT  
	...
	COUNT(DISTINCT ID_CLIENTE)  AS FREQ_MEDIA
FROM TB_VENDA
...
SELECT TOP 10 
	NM_PRODUTO, 
	SUM( QT_ITEM * (VL_PRECOUNITARIO - VL_DESCONTOUNITARIO) ) as VLR_VENDIDO,
...
-- EXIBIR VENDAS POR ANO E TRIMESTRES, SOMENTE TRIMESTRES COM MAIS DE 5 MILHÕES EM VENDAS. 
SELECT	
	YEAR(DT_VENDA) AS ANO,
	DATEPART(QUARTER,DT_VENDA) AS TRI,
	SUM(VL_SUBTOTAL) AS TOT_VENDA
...
-- EXIBIR VENDAS POR MÊS, SOMENTE DOS MESES COM 31 DIAS COM MAIS DE 1 MILHÃO E 700 MIL EM VENDAS. 
SELECT	
	YEAR(DT_VENDA) AS ANO,
