...
FROM TB_PRODUTO
ORDER BY VALOR_LUCRO DESC; -- ou ORDER BY 5 DESC;


/*5. Exiba quais as cidades de São Paulo cadastradas nos endereços. Se a cidade possuir
mais de um endereço, exibir uma única vez.*/
SELECT  DISTINCT 
	NM_CIDADE 
FROM TB_ENDERECO
WHERE NM_ESTADO = 'SP';
...
/*7. Retornar os produtos com custo superior a R$1.000 que começaram a ser vendidos em
2021.*/
SELECT * FROM TB_PRODUTO 
WHERE VL_PRECOCUSTO > 1000 
AND DT_INICIOVENDA BETWEEN '2021-01-01' AND '2021-12-31'; 
-- OU AND DT_INICIOVENDA >= '2021-01-01'; 


/*8. Exibir o nome das categorias que não possuem categorias superiores.*/
SELECT 
	NM_CATEGORIA, 
	ID_CATEGORIA_SUPERIOR 
FROM TB_CATEGORIA
